...
function su_addon_active( $addons ) {
	...
	$paths = array(
		'maker' => 'psource-shortcodes-maker/psource-shortcodes-maker.php',
		'skins' => 'psource-shortcodes-skins/psource-shortcodes-skins.php',
		'extra' => 'psource-shortcodes-extra/psource-shortcodes-extra.php',
	);
...
function su_skins_link() {
	if ( su_addon_active( 'skins' ) ) return sprintf( '<br><strong>%s</strong><br><strong>%s</strong>', __( 'Additional skins successfully installed', 'psource-shortcodes' ), __( 'Open dropdown to choose one of new styles', 'psource-shortcodes' ) );
	else return sprintf( '<br><a href="http://gndev.info/sus" target="_blank">%s &rarr;</a>', __( 'Get more styles', 'psource-shortcodes' ) );
}
...
class Su_Tools {
	...
	// 		// Check for code
	// 		if ( !$code ) die( '<p class="su-examples-error">' . __( 'Example code does not found, please check it later', 'psource-shortcodes' ) . '</p>' );
	// 		// Clean-up the code
	...
	// 			echo '<div class="su-examples-code"><span class="su-examples-get-code button"><i class="fa fa-code"></i>&nbsp;&nbsp;' . __( 'Get the code', 'psource-shortcodes' ) . '</span><textarea rows="' . $rows . '">' . esc_textarea( $chunk ) . '</textarea></div>';
	// 		}

...
<?php
	...
	public static function settings() {
		self::access();
		// Param check
		if ( empty( $_REQUEST['shortcode'] ) ) wp_die( __( 'Shortcode nicht angegeben', 'upfront-shortcodes' ) );
		...
		$actions = apply_filters( 'su/generator/actions', array(
				'insert' => '<a href="javascript:void(0);" class="button button-primary button-large su-generator-insert"><i class="sui sui-check"></i> ' . __( 'Shortcode einfügen', 'upfront-shortcodes' ) . '</a>',
				'preview' => '<a href="javascript:void(0);" class="button button-large su-generator-toggle-preview"><i class="sui sui-eye"></i> ' . __( 'Live Vorschau', 'upfront-shortcodes' ) . '</a>'
			) );
		// Shortcode header
		$return = '<div id="su-generator-breadcrumbs">';
		$return .= apply_filters( 'su/generator/breadcrumbs', '<a href="javascript:void(0);" class="su-generator-home" title="' . __( 'Klicke hier, um zur Liste der Shortcodes zurückzukehren', 'upfront-shortcodes' ) . '">' . __( 'Alle Shortcodes', 'upfront-shortcodes' ) . '</a> &rarr; <span>' . $shortcode['name'] . '</span> <small class="alignright">' . $shortcode['desc'] . '</small><div class="su-generator-clear"></div>' );
		$return .= '</div>';
		// Shortcode note
	...
	public static function preview() {
		...
		echo '<h5>' . __( 'Vorschau', 'upfront-shortcodes' ) . '</h5>';
	...
	public static function access() {
		if ( !self::access_check() ) wp_die( __( 'Zugriff verweigert', 'upfront-shortcodes' ) );
	}
...
<div class="su-generator-presets alignright" data-shortcode="<?php echo sanitize_key( $_REQUEST['shortcode'] ); ?>">
	...
	public static function presets_list( $shortcode = false ) {
		...
		if ( is_array( $presets ) && count( $presets ) ) {
			...
			// Hide default text
			echo sprintf( '<b style="display:none">%s</b>', __( 'Voreinstellungen nicht gefunden', 'upfront-shortcodes' ) );
		}
		// Presets doesn't found
		else echo sprintf( '<b>%s</b>', __( 'Voreinstellungen nicht gefunden', 'upfront-shortcodes' ) );
	}
...
class UpFront_Shortcodes_Generator extends Su_Generator {

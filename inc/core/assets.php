...
class Su_Assets {
	...
	public static function register() {
		...
		wp_localize_script(
			...
			'SUShortcodesL10n',
			array(
				'noPreview'     => __( 'Dieser Shortcode funktioniert in der Live-Vorschau nicht. Bitte füge es in den Editor und die Vorschau auf der Webseite ein.', 'upfront-shortcodes' ),
				'magnificPopup' => array(
					'close'   => __( 'Schließen (Esc)', 'upfront-shortcodes' ),
					'loading' => __( 'Laden...', 'upfront-shortcodes' ),
					'prev'    => __( 'Zurück (linke Pfeiltaste)', 'upfront-shortcodes' ),
					'next'    => __( 'Weiter (rechte Pfeiltaste)', 'upfront-shortcodes' ),
					// translators: %1$s of %2$s represents image counter in lightbox, will be replaced with "1 of 5"
					'counter' => sprintf( __( '%1$s von %2$s', 'upfront-shortcodes' ), '%curr%', '%total%' ),
					'error'   => sprintf(
						// translators: %1$s and %2$s will be replace <a> and </a> tags
						__( 'Fehler beim Laden des Inhalts. %1$sLink öffnen%2$s' ),
	...
	public static function skin_url( $file = '' ) {
		...
		$url = ( !$skin || $skin === 'default' ) ? plugins_url( 'assets/css/', SU_PLUGIN_FILE ) : $uploads . '/upfront-shortcodes-skins/' . $skin;

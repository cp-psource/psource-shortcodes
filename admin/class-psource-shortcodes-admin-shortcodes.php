 ...
 */
final class PSOURCE_Shortcodes_Admin_Shortcodes extends PSOURCE_Shortcodes_Admin {
	...
	public function admin_menu() {
		...
		$this->add_submenu_page(
			'psource-shortcodes',
			__( 'Available shortcodes', 'psource-shortcodes' ),
			__( 'Available shortcodes', 'psource-shortcodes' ),
			$this->get_capability(),
			'psource-shortcodes',
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style( 'psource-shortcodes-admin', $this->get_plugin_url() . 'admin/css/admin.css', array( 'font-awesome' ), $this->get_plugin_version() );
	...
	protected function get_groups() {
		...
		$groups['all'] = __( 'All shortcodes', 'psource-shortcodes' );
	...
	protected function get_possible_values( $args ) {
		...
		$image_sources = array(
			'media: 1,2,3'       => __( 'Media file IDs', 'psource-shortcodes' ),
			'posts: 1,2,3'       => __( 'Post IDs', 'psource-shortcodes' ),
			'posts: recent'      => __( 'Recent posts', 'psource-shortcodes' ),
			'category: 1,2,3'    => __( 'Category IDs', 'psource-shortcodes' ),
			'taxonomy: book/3,5' => __( 'Taxonomy term slug / term IDs', 'psource-shortcodes' ),
		);

		$possible = array(
			'select'          => $this->implodef( '<br>', $args['values'], '%1$s (%2$s)' ),
			'color'           => __( '#HEX color', 'psource-shortcodes' ),
			'text'            => __( 'Any text value', 'psource-shortcodes' ),
			'border'          => __( 'CSS border shorthand property', 'psource-shortcodes' ),
			'shadow'          => __( 'CSS box/text-shadow shorthand property', 'psource-shortcodes' ),
			'extra_css_class' => __( 'CSS class name(s) separated by space(s)', 'psource-shortcodes' ),
			'post_type'       => __( 'Post type slug(s) separated by comma(s)', 'psource-shortcodes' ),
			'taxonomy'        => __( 'Taxonomy slug(s) separated by comma(s)', 'psource-shortcodes' ),
			'term'            => __( 'Term slug(s) separated by comma(s)', 'psource-shortcodes' ),
			'upload'          => __( 'The URL of uploaded file', 'psource-shortcodes' ),
			'bool'            => sprintf( __( '%s or %s', 'psource-shortcodes' ), 'yes', 'no' ),
			'number'          => sprintf( __( 'Number from %s to %s', 'psource-shortcodes' ), $args['min'], $args['max'] ),
			'slider'          => sprintf( __( 'Number from %s to %s', 'psource-shortcodes' ), $args['min'], $args['max'] ),
			'icon'            => sprintf( '%s. %s: <em>icon: star</em>, <em>http://example.com/icon.png</em>', __( 'FontAwesome icon name (with "icon:" prefix) or icon image URL', 'psource-shortcodes' ), __( 'Examples', 'psource-shortcodes' ) ),
	...
	protected function get_default_value( $args ) {
		...
		return sprintf( '<p class="description">%s</p>', __( 'none', 'psource-shortcodes' ) );
